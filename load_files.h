	...
	int run7 = 0, int run8 = 0, int run9 = 0)
{
	...
	for (int fileIndex = 0; fileIndex < count; fileIndex++) {
        	...
        	// Open .root data file
		filename = Form("/home/brady/Data/slac-root-files/run_%d.root", runs[fileIndex]);
...
TH1F* load_files_ped_corrected(int pedestal = 0, float scale = 0.0, int adc_range = 1, int binWidth = 1, int chan = 2, int run0 = 0, 
	int run1 = 0, int run2 = 0, int run3 = 0, 
	int run4 = 0, int run5 = 0, int run6 = 0,
	int run7 = 0, int run8 = 0, int run9 = 0)
{
	...
	for (int fileIndex = 0; fileIndex < count; fileIndex++) {
		...
		float value;
        	for (Int_t entry = 0; entry < branch->GetEntries(); entry++) {
        	        branch->GetEntry(entry);
			value = leaf->GetValue() - pedestal;
			if (value >= 0) h_QDC->Fill(value);
        	}
		file->Close();
	}

	h_QDC->GetXaxis()->SetLimits(0.0, (float)(MAX_BIN) / scale);
