...
float benchmark_compare_voltage_1a_3(int binWidth = 8){
	...
	qdc[index]->SetTitle("Benchmarking 1A (5.5 GeV): 3 tungsten");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 332, 333);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A (5.5 GeV): 3 tungsten");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	legend->AddEntry(qdc[0], " 1000", "l");
	legend->AddEntry(qdc[1], " 950", "l");
	legend->Draw();
