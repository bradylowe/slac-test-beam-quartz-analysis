...
float benchmark_compare_stacks_1a(int binWidth = 2){
	...
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 339, 340);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A: Compare stacks");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 329, 330);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A: Compare stacks");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	if (false) {
        ...
        sig = GetSignalFromRun(run) / onePEsig;
        rms = GetSignalRmsFromRun(run) / onePEsig;
        qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 319, 320, 321);
        qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A: Compare stacks");
        qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
        qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 300, 301);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A: Compare stacks");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 293, 294, 295);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A: Compare stacks");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	legend->AddEntry(qdc[4], "0 stacks (1200 Volts)", "l");
	legend->AddEntry(qdc[3], "1 stacks (1100 Volts)", "l");
	legend->AddEntry(qdc[2], "2 stacks (1000 Volts)", "l");
	legend->AddEntry(qdc[1], "3 stacks (1000 Volts)", "l");
	legend->AddEntry(qdc[0], "4 stacks (950 Volts)", "l");
	legend->Draw();
