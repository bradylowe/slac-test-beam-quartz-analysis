...
float benchmark_compare_voltage_1a_0(int binWidth = 2){
	...
	qdc[index]->SetTitle("Benchmarking 1A (5.5 GeV): no tungsten");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 298);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1A (5.5 GeV): no tungsten");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	legend->AddEntry(qdc[0], " 1300", "l");
	legend->AddEntry(qdc[1], " 1200", "l");
	legend->AddEntry(qdc[2], " 1100", "l");
	legend->Draw();
