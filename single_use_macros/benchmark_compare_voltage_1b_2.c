...
float benchmark_compare_voltage_1b_2(int binWidth = 8){
	...
	qdc[index]->SetTitle("Benchmarking 1B (5.5 GeV): 2 tungsten");
	...
	sig = GetSignalFromRun(run) / onePEsig;
	rms = GetSignalRmsFromRun(run) / onePEsig;
	qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 2, run, 290);
	qdc[index]->SetLineColor(color);
	qdc[index]->SetTitle("Benchmarking 1B (5.5 GeV): 2 tungsten");
	qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
	qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	legend->AddEntry(qdc[0], " 1200", "l");
	legend->AddEntry(qdc[1], " 1150", "l");
