...
float benchmark_compare_energy_1b(int binWidth = 2){
	...
	qdc[index]->SetTitle("Benchmarking 1B: Full stack");
	...
	stats[index]->DrawClone();
        ...
        sig = GetSignalFromRun(run) / onePEsig;
        rms = GetSignalRmsFromRun(run) / onePEsig;
        qdc[index] = load_files_ped_corrected(pedestal, onePEsig, 1, binWidth, 0, run);
        qdc[index]->SetLineColor(color);
        qdc[index]->SetTitle("Benchmarking 1B: Full stack");
        qdc[index]->GetXaxis()->SetTitle("Photo-electrons");
        qdc[index]->GetYaxis()->SetTitle("Counts");
	...
	legend->AddEntry(qdc[0], "8.0 GeV (900 Volts)", "l");
	legend->AddEntry(qdc[1], "5.5 GeV (1000 Volts)", "l");
	legend->AddEntry(qdc[2], "3.0 GeV (950 Volts)", "l");
	legend->Draw();
